...
export default async function handler(req, res) {	
	...
	try {
		...
		await fsPromises.writeFile("C:/Users/Fahim/Documents/Web projects/invoice-app/model/invoices.json", JSON.stringify(invoiceDB.invoices));
		let targetInvoice = invoiceDB.invoices.find((invoice) => invoice.id === id);
		res.status(200).json(targetInvoice).end();
	} catch (error) {
		console.log(error);
